...
画板关闭前，Codex 会等待你的操作。需要多张草图时，按 `Ctrl+N` 保存当前非空页面并继续绘制；最多保存 6 张，空白页不会加入结果。完成后，Skill 会返回有序 PNG 路径列表和一份可选的整体说明；Codex 随后逐张读取并在对话中展示，再继续回答原始问题。
