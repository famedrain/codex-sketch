...
一个面向 Windows Codex 的极简草图 Skill。它打开本地画板，让你直接用鼠标画出页面结构、流程或示意图；支持在同一次调用中连续绘制多张草图，完成后由 Codex 统一读取并继续处理。
...
- 顶部直接显示画笔、橡皮和自动修复的当前状态；也可以点击切换
- 底部显示当前页码，并可点击左右箭头翻页
- 页面在内存中保持可编辑，可返回任意一页继续画、逐页撤销/重做或删除当前页
- `Ctrl+N` 新建下一张，单次最多 6 张
- 可选自动修复直线、圆/椭圆和近似水平或垂直的矩形，黑笔与红笔使用同一识别规则
...
如果目标目录已有旧版，安装脚本默认停止，避免意外覆盖。确认用最新版替换旧版时使用：

```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\install.ps1 -Force
```

`-Force` 会删除目标目录中的旧版再安装最新版，不保留可能被 Codex 重复识别的备份 Skill。安装或升级后，请新建一个 Codex 任务，确保新的 Skill 配置被加载。
...
画板关闭前，Codex 会等待你的操作。需要多张草图时，按 `Ctrl+N` 新建页面；用底部箭头或 `PageUp` / `PageDown` 随时返回前后页继续修改。最多保留 6 页，提交时自动忽略空白页。完成后，Skill 才会把非空页面依次导出，返回有序 PNG 路径列表和一份可选的整体说明；Codex 随后逐张读取并在对话中展示，再继续回答原始问题。
...
| `Ctrl+Delete` | 二次确认后删除当前页；只剩一页时清空该页 |
| `Ctrl+Z` | 撤销；自动修复后第一次恢复原笔迹，第二次删除 |
| `Ctrl+Y` / `Ctrl+Shift+Z` | 重做 |
| `Ctrl+Backspace` | 二次确认后清空画板 |
| `Ctrl+Enter` | 打开说明输入框；再次按下后完成 |
| `Ctrl+Shift+Enter` | 跳过说明并快速完成 |
| `Esc` | 关闭帮助、返回画板或取消；有未提交内容时会再次确认 |

自动修复会先按绘制距离重新采样，因此鼠标快慢不会明显影响识别；正常的小缺口和抖动可以容忍。水滴、涂鸦、自交线和缺口过大的圆弧会保留原始笔迹，避免被强行改成其他形状。
...
- 多图结果通过兼容字段 `path`（第一张）和 `paths`（全部图片，按绘制顺序）返回
- 翻页时只在内存中保存可编辑笔迹，只有最终提交才会生成 PNG
